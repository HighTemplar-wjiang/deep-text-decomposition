...
def run_eval(args):
    ...
    if args.dataset == 'dogs':
        ...
        datadir = r'./datasets/letters/val'
        val_dirs = glob.glob(os.path.join(datadir, '*.jpg'))
        datadir_letters = r'./datasets/letters/val'
        val_dirs_letters = glob.glob(os.path.join(datadir_letters, '*.jpg'))
    ...
    for idx in range(len(val_dirs)):
        ...
        if args.dataset == 'dogs':
            ...
            img1 = cv2.imread(val_dirs[idx], cv2.IMREAD_COLOR)
            img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
            img2 = cv2.imread(val_dirs_letters[-idx-1], cv2.IMREAD_COLOR)
            img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)
