    ...
    def transform(self, img):
        ...
        if self.with_random_crop and random.random() > 0.5:
            i, j, h, w = transforms.RandomResizedCrop(size=self.img_size). \
                get_params(img=img, scale=(0.8, 1.0), ratio=self.crop_ratio)
            img = TF.resized_crop(
...
def mixture(img1, img2, mix_mode='linear', gaussian_blur=None, downsample_size=0):
    if mix_mode == 'linear':
        ...
        gt1 = img1
        gt2 = img2
        img_mix = gt1 * alpha1 + gt2 * alpha2
    elif mix_mode == "uniform":
        alpha1 = 1.0
        alpha2 = 1.0
        gt1 = img1 * alpha1
        gt2 = img2 * alpha2
    ...
    # Image processing.
    original_size = img_mix.size()
    img_mix = TF.to_pil_image(img_mix)
    if downsample_size:
        target_size = np.random.randint(downsample_size, original_size[1])
        img_mix = TF.resize(img_mix, [target_size, target_size])
        # img_mix = TF.resize(img_mix, list(original_size[1:]))
        # img_mix = TF.to_tensor(img_mix)

    if gaussian_blur:
        blurred = np.random.randint(0, 2)
        if blurred:
            # img_mix = gaussian_blur.forward(img_mix)
            img_mix = img_mix.filter(gaussian_blur)

    # Add background noise.
    # normal_noise = np.clip(np.random.normal(loc=0.1, scale=0.05, size=img_mix.size), a_min=0.0, a_max=1.0)
    # normal_noise = np.tile(normal_noise, (3, 1, 1))
    # img_mix += normal_noise
...
class TwoFoldersUnmixDataset(Dataset):
    ...
    def __init__(self, root_dir_1, root_dir_2, img_size, mix_mode='linear', suff='.jpg',
                 is_train=True, is_blurred=False, downsample_size=0):
        ...
        if is_blurred:
            # self.gaussian_blur = torchvision.transforms.GaussianBlur(kernel_size=5, sigma=5.0)
            self.gaussian_blur = PIL.ImageFilter.GaussianBlur(radius=1)
        else:
    ...
    def __getitem__(self, idx):
        ...
        data = mixture(img1, img2, self.mix_mode, self.gaussian_blur, self.downsample_size)
        data["input"] = self.augm.transform(data["input"])
...
def get_loaders(args):
    if args.dataset == "texts":
        training_set = TwoFoldersUnmixDataset(
            root_dir_1=r"./train_data/" + args.subset,
            root_dir_2=r"./train_data/" + args.subset,
            # root_dir_1=r"./test_data",
            # root_dir_2=r"./test_data",
            img_size=128, suff='.bmp', mix_mode=args.mix_mode, is_train=True,
            is_blurred=args.enable_gaussian_blur, downsample_size=args.downsample_size
        )
        val_set = TwoFoldersUnmixDataset(
            root_dir_1=r"./test_data/" + args.subset,
            root_dir_2=r"./test_data/" + args.subset,
            img_size=128, suff='.bmp', mix_mode=args.mix_mode, is_train=False,
            is_blurred=args.enable_gaussian_blur, downsample_size=args.downsample_size
        )
    else:
        raise NotImplementedError(
            'Wrong dataset name %s (choose one from [texts])'
            % args.dataset)
...
def make_numpy_grid(tensor_data, enhance=False):
    ...
    vis = utils.make_grid(tensor_data, nrow=12)
...
class SquarePad:
    def __call__(self, image):
        _, h, w = image.size()
        max_wh = np.max([h, w])
        tb_pad = int((max_wh - h) / 2)
        lr_pad = int((max_wh - w) / 2)
        padding = (lr_pad, lr_pad, tb_pad, tb_pad)
        return F.pad(image, padding, value=1.0, mode='constant')


class CenterPad:

    def __init__(self, target_size, downsample_ratio=0.5):
    ...
    def __call__(self, image):
        ...
        # Downscale to fit.
        center_height = int(self.target_size[0] * self.downsample_ratio)
        center_width = int((w / h) * center_height)
        output_image = TF.resize(image, [center_height, center_width])

        # Padding.
        tb_pad = int((self.target_size[0] - center_height) / 2)
        lr_pad = int((self.target_size[1] - center_width) / 2)
        padding = (lr_pad, lr_pad, tb_pad, tb_pad)
        output_image = F.pad(output_image, padding, value=1.0, mode="constant")

        return TF.resize(output_image, self.target_size)
